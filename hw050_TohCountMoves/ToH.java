...
public class ToH {
    ...
    /* 
     * I am asked to sum the counting numbers from 1 through the given argument (n), inclusive
     * When I am asked to sum the counting numbers from 1 through the given argument (n), inclusive,
     * if n is equal to 1, then return 1
     * the recursive abstraction can follow these instructions to
     * calculate the sum of the counting numbers from 1 through one less than the given argument (n - 1), inclusive, plus the given argument (n)
     */
    static int sumCountingThrough (int n ) {
        if (n == 1) 
            return 1;
        return n + sumCountingThrough(n -1);
    }
