...
public class StringLooper {
    ...
    public static int indexOf1( String seek1, String within) {
        int index = 0;
        while(index < within.length()) {
            if (within.substring(index, index + 1).equals(seek1)) {
                return index;
            }
            index ++;
        }
    ...
    public static int indexOfSubstring( String seek, String within) {
        int length = seek.length();
        int withinIndex = 0;
        

        while (withinIndex + length - 1 < within.length())  {
            boolean ok = true;
            int seekIndex = 0;
            while (seekIndex < length) {
                if (!seek.substring(seekIndex,seekIndex + 1).equals
                   (within.substring(withinIndex + seekIndex, withinIndex + seekIndex + 1))) 
                   {
                    ok = false;
                }
                seekIndex++;
